...
module clk_int_div #(
  ...
  parameter int unsigned DEFAULT_DIV_VALUE = 1'b0,
...
//    div_ready_o         = 1'b0;
    ...
    case (clk_gate_state_q)
      IDLE: begin
        gate_en_d     = 1'b1;
        toggle_ffs_en = 1'b1;
        if (div_valid_i && (div_i != div_q)) begin
...
//          div_ready_o             = 1'b1;
          ...
          clk_div_bypass_en_d     = (div_i == {DIV_VALUE_WIDTH{1'b0}}) || (div_i == {{DIV_VALUE_WIDTH-1{1'b0}},1'b1});
...
/*        if(cycle_cntr_q == 0) begin
        ...
        end else */if (clk_div_bypass_en_q || cycle_cntr_q == div_q - {{DIV_VALUE_WIDTH-1{1'b0}},1'b1}) begin
          clk_gate_state_d = IDLE;
        end
      end
  ...
  always_comb begin
    ...
    if (clear_cycle_counter) begin
      cycle_cntr_d = {DIV_VALUE_WIDTH{1'b0}};
    end else begin
      if (cycle_counter_en) begin
        ...
        if (clk_div_bypass_en_q || (cycle_cntr_q == div_q-{{DIV_VALUE_WIDTH-1{1'b0}},1'b1})) begin
          cycle_cntr_d = {DIV_VALUE_WIDTH{1'b0}};
        end else begin
          cycle_cntr_d = cycle_cntr_q + {{DIV_VALUE_WIDTH-1{1'b0}},1'b1};
        end
      end
    end
  end
  ...
  always_ff @(posedge clk_i, negedge rst_ni) begin
    if (!rst_ni) begin
      cycle_cntr_q <= {DIV_VALUE_WIDTH{1'b0}};
    end else begin
...
//  assign cycl_count_o = cycle_cntr_q;
  ...
  always_ff @(posedge clk_i, negedge rst_ni) begin
    if (!rst_ni) begin
      t_ff1_q = 1'b0; // Intentional blocking assignment! Do not replace!
    end else begin
      if (t_ff1_en) begin
  ...
  always_ff @(negedge clk_i, negedge rst_ni) begin
    if (!rst_ni) begin
      t_ff2_q = 1'b0; // Intentional blocking assignment! Do not replace!
    end else begin
      if (t_ff2_en) begin
  ...
  always_comb begin
    ...
    if (!clk_div_bypass_en_q && toggle_ffs_en) begin
      if (use_odd_division_q) begin
        t_ff1_en = (cycle_cntr_q == {DIV_VALUE_WIDTH{1'b0}})? 1'b1: 1'b0;
        t_ff2_en = ({1'b0,cycle_cntr_q} == (div_q+{{DIV_VALUE_WIDTH-1{1'b0}},1'b1})/({{DIV_VALUE_WIDTH-3{1'b0}},4'h2}))? 1'b1: 1'b0;
      end else begin
        t_ff1_en = (cycle_cntr_q == {DIV_VALUE_WIDTH{1'b0}} || {1'b0,cycle_cntr_q} == div_q/({{DIV_VALUE_WIDTH-3{1'b0}},4'h2}))? 1'b1: 1'b0;
      end
    end
  end
  ...
  tc_clk_gating #(
    .IS_FUNCTIONAL(1'b1) // The gate is required to prevent glitches during
