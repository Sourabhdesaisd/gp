module rx_fsm #(
  ...
  parameter REG_WIDTH  = 8 
   ...
   // output logic rxed_valid,
    ...
    typedef enum logic [2:0]{IDLE, START, DATA, PARITY, STOP, WAIT_STATE} state_t;
    state_t state;
		...
		//	end
			...
			//	data_out <='0;
			 ...
			 data_valid  <= '0;
                    ...
                    if (data_bits == 4'd5 && (sample_cnt ==(cnt- 5'b01)))
                        shift_reg <= {3'b000, rx, shift_reg[4:1]};
                    else if (data_bits == 4'd6 && (sample_cnt ==(cnt- 5'b01)))
                        shift_reg <= {2'b00, rx, shift_reg[5:1]};
                    else if (data_bits == 4'd7 && (sample_cnt ==(cnt- 5'b01)))
                        shift_reg <= {1'b0, rx, shift_reg[6:1]};
		     else if(data_bits == 4'd8 && (sample_cnt ==(cnt- 5'b01)))
                        shift_reg <= {rx, shift_reg[7:1]};

                    else
                        shift_reg <= shift_reg;

                    if ((sample_cnt ==(cnt- 5'b01) ))
                        parity_calc <= parity_calc ^ rx;
		  if(sample_cnt ==(cnt- 5'b01) )
                    bit_cnt <= bit_cnt + 4'b01;

                    if (bit_cnt == data_bits-4'b01  && (sample_cnt ==(cnt- 5'b01)))begin
			if(LCR[3]) begin
				state<= PARITY;
				break_int1 <='0;
				framing_err <= '0;
