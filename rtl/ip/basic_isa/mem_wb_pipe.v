module mem_wb_pipe (
 ...
 //   input          flush,     // bubble insertion (1 = insert NOP/bubble)
    ...
    input          memtoreg_in,
    input          halt_valid_wb_i, 
    input          exception_flush_i,
    input           is_branch_jump_mem,
    input           last_commit_pc_valid_in,

    input [31:0]        inst_mem_i,

    output reg [31:0]   inst_wb_o,
   ...
   /* // Optional: pass-through branch info for observation (not for redirect)
    ...
    output reg          is_branch_jump_wb ,
    output reg          last_commit_pc_valid
...
//    assign data_forward_wb = (memtoreg_out) ? load_data_out : alu_result_out;
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            memtoreg_out     <= 1'b0;
            halt_valid_wb_o  <= 1'b0;
            last_commit_pc_valid <= 1'b0;
            is_branch_jump_wb <= 1'b0;
         /*   modify_pc_out    <= 1'b0;
	...
	begin
            ...
            inst_wb_o       <= inst_mem_i;
            pc_wb           <=  pc_mem ;
            load_data_out    <= load_data_in;
            rd_wb           <= rd_in;
            wb_reg_file_out  <= wb_reg_file_in;
            memtoreg_out     <= memtoreg_in;
            halt_valid_wb_o  <= halt_valid_wb_i;
            last_commit_pc_valid <= last_commit_pc_valid_in;
            is_branch_jump_wb <= is_branch_jump_mem;
