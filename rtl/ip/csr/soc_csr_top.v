...
input                               mret_valid_i        			    ,
output [7:0] 		                int_active_level_priority_o	,
...
output  [19:0]                      mtval_o                             ,
input   [DATA_WIDTH-1:0]            mtval_i                             ,
input                               mtval_write_valid                   ,
output  [15:0]                      mie_to_int_ctrl_o
//output 				reset_uart_o	
...
//wire [DATA_WIDTH-1:0] uart_reset_csr_o;

///////////////////// CSR WRITE ENABLE LOGIC ////////////////////
//wire data_valid;
//assign data_valid = (csr_write_data_in != 32'd0) ? 1'b1 : 1'b0 ;
wire set_clear;
assign set_clear =  (csr_write_en_in && (csr_set_bit_in || csr_clear_bit_in)) ? ((csr_write_data_in != 32'd0) ? 1'b1 : 1'b0) : 1'b0 ;

wire csr_write_en;
assign csr_write_en = (csr_write_en_in && (~(csr_set_bit_in || csr_clear_bit_in))) ? 1'b1 : set_clear ;

////////////////////////////////////////////////////////////////


wire dbg_mode_csr_read_en_w;
assign dbg_mode_csr_read_en_w = dbg_mode_csr_addr_i[15:12] ? 1'b0 : dbg_mode_read_en_i ;
...
wire [DATA_WIDTH-1:0] int_active_level_priority_w;
assign int_active_level_priority_o = int_active_level_priority_w[7:0] ;
assign csr_mcause_o = mcause[7:0];

assign mie_to_int_ctrl_o = mie[31:16]; // to Interrupt Controller
assign mtval_o          = mtval[19:0];

wire [DATA_WIDTH-1:0] dcsr_w  ;
//wire [INSTRUCTION_WIDTH-1:0] dpc_w   ;
//assign reset_uart_w = uart_reset_csr_o[0];

reg halt_req_r,halt_req_temp_r;

always@(posedge csr_clk or negedge csr_rstn)
begin
	if(!csr_rstn)
	begin
		halt_req_temp_r <= 1'b0;        
		halt_req_r <= 1'b0;
	end
	else
	begin
		halt_req_temp_r <= haltreq_valid_i;        
		halt_req_r <= halt_req_temp_r;
	end
end

reg rst_halt_req_r , rst_halt_req_temp_r;

always@(posedge csr_clk or negedge csr_rstn)
begin
	if(!csr_rstn)
	begin
		rst_halt_req_temp_r <= 1'b0;        
		rst_halt_req_r <= 1'b0;
	end
	else
	begin
		rst_halt_req_temp_r <= reset_haltreq_valid_i;        
		rst_halt_req_r      <= rst_halt_req_temp_r;
	end
end
...
//.mip_i 			(mip 			),
.satp_i     (satp), //Supervisor Address Translation and Protection
...
.int_active_level_priority_i(int_active_level_priority_w),
//.data_mem_max_addr_i(data_mem_max_addr_o),
.dcsr_i         (dcsr_w),
.dpc_i          (dpc_o ),
.dbg_mode_valid_i                   (dbg_mode_valid_i),
.dbg_mode_csr_addr_i                (dbg_mode_csr_addr_i[11:0]),
...
.int_active_level_priority_o(int_active_level_priority_w),
...
.reset_haltreq_valid_i   (rst_halt_req_r        ),
.dcsr_o                  (dcsr_w                ),
.dpc_o                   (dpc_o                 ),
.dbg_csr_write_en_i      (dbg_mode_write_en_i    ),
.dbg_csr_addr_i          (dbg_mode_csr_addr_i[11:0]),
