...
//output          wdt_reset_o             ,
output   [7:0]  current_int_id_o    ,
output reg  [31:0] trace_data_int_o    ,
output reg  [7:0]  trace_event_int_o   ,
output 		    interrupt_request_o		    //interrupt request signal to core	

);

wire [7:0]  ack_int_id_to_csr_w         ;
wire [7:0]  highest_pending_lvl_pr_w    ;
wire [31:0] soc_mmr_read_data_w         ;
...
//--------------------------------------------------------
reg write_en_sync1_r, write_en_sync2_r;
//reg write_en_sync3_r;
reg read_en_sync1_r, read_en_sync2_r;
//reg read_en_sync3_r;

always@ (posedge soc_clk or negedge soc_rst) begin
if(!soc_rst) begin
    write_en_sync1_r   <= 1'b0; 
    write_en_sync2_r   <= 1'b0; 
    //write_en_sync3_r   <= 1'b0; 
    read_en_sync1_r    <= 1'b0; 
    read_en_sync2_r    <= 1'b0; 
    //read_en_sync2_r    <= 1'b0; 
end
else begin
    write_en_sync1_r   <= soc_mmr_write_en_i; 
    write_en_sync2_r   <= write_en_sync1_r;
    //write_en_sync3_r   <= write_en_sync2_r;
    read_en_sync1_r    <= soc_mmr_read_en_i;
    read_en_sync2_r    <= read_en_sync1_r;
    //read_en_sync2_r    <= read_en_sync2_r;
    end
end

//--------------- MMR WRITE SYNC -------------------------
reg [31:0] sync_write_data_w;
reg [15:0] sync_write_addr_w;
reg        sync_write_en_w;

always@ (*) begin
if (write_en_sync2_r) begin
    sync_write_data_w  = soc_mmr_write_data_i        ;
    sync_write_addr_w  = soc_mmr_write_addr_i        ;
    sync_write_en_w    = write_en_sync2_r            ;
    end
else begin
    sync_write_data_w  = 32'd0                       ;
    sync_write_addr_w  = 16'd0                       ;
    sync_write_en_w    = 1'b0                        ;
    end
end
//--------------- MMR READ SYNC --------------------------

//reg [31:0] sync_read_data_w;
reg [15:0] sync_read_addr_w;
reg        sync_read_en_w;

always@ (*) begin
if (read_en_sync2_r) begin
    sync_read_addr_w  = soc_mmr_read_addr_i         ;
    sync_read_en_w    = read_en_sync2_r             ;
    end
else begin
    sync_read_addr_w  = 16'd0                       ;
    sync_read_en_w    = 1'b0                        ;
    end
end

reg [31:0]  read_data_hold_r;
reg         read_rsp_toggle_r;

always@ (posedge soc_clk or negedge soc_rst) begin
    if (!soc_rst) begin
        read_data_hold_r <= 32'd0;
        read_rsp_toggle_r <= 1'b0;
    end
    else if (sync_read_en_w) begin
        read_data_hold_r   <= soc_mmr_read_data_w;
        read_rsp_toggle_r  <= ~read_rsp_toggle_r ;
    end
end

assign soc_mmr_read_data_o =    read_data_hold_r    ;
assign soc_read_rsp_o     =    read_rsp_toggle_r;
...
.soc_mmr_write_en_i		        (sync_write_en_w		    ),//from core
.soc_mmr_write_addr_i		    (sync_write_addr_w		    ),//from core
.soc_mmr_write_data_i		    (sync_write_data_w		    ),//from core
.soc_mmr_read_en_i		        (sync_read_en_w		        ),//from core
.soc_mmr_read_addr_i		    (sync_read_addr_w		    ),//from core
.soc_mmr_read_data_o		    (soc_mmr_read_data_w		),//to core
...
.soc_ack_int_id_o		        (ack_int_id_to_csr_w		    ),//to processor ack
...
.soc_ack_id_in       	(ack_int_id_to_csr_w		),//interrupt id of acknowledged interrupt
...
localparam      INT_REQ         = 8'h01         ;
localparam      INT_ACCEPTED    = 8'h02         ;
localparam      INT_END         = 8'h03         ;
localparam      MMR_READ        = 8'h04         ;
localparam      MMR_WRITE       = 8'h05         ;

always@(posedge soc_clk or negedge soc_rst) begin

    if(!soc_rst) begin
        trace_event_int_o   <= 8'h00;
        trace_data_int_o    <= 32'd0;
    end
    else if (sync_read_en_w) begin
        trace_event_int_o   <= MMR_READ;
        trace_data_int_o    <= {8'd0,soc_mmr_read_data_w[7:0],sync_read_addr_w};
    end
    else if (sync_write_en_w) begin
        trace_event_int_o   <= MMR_WRITE;
        trace_data_int_o    <= {8'd0,sync_write_data_w[7:0],sync_write_addr_w};
    end
    else if (interrupt_request_w) begin
        trace_event_int_o   <= INT_REQ;
        trace_data_int_o    <= {16'd0,highest_pending_lvl_pr_w,soc_ack_int_id_w};
    end
    else if (soc_ack_read_valid_en) begin
        trace_event_int_o   <= INT_ACCEPTED;
        trace_data_int_o    <= {24'd0,ack_int_id_to_csr_w};
    end
    else if (soc_eoi_valid_i) begin
        trace_event_int_o   <= INT_END;
        trace_data_int_o    <= {24'd0,soc_eoi_id_i};
    end
    else begin
        trace_event_int_o   <= 8'h00;
        trace_data_int_o    <= 32'd0;
...
assign  soc_ack_int_id_o    = ack_int_id_to_csr_w;
//assign  soc_mmr_read_data_o = soc_mmr_read_data_w   ; 
assign  interrupt_request_o = interrupt_request_w   ;
assign  current_int_id_o    = soc_ack_int_id_w      ;
//assign  trace_data_int_o  = 32'd0;
//assign  trace_event_int_o = 8'd0;
