module i2c_master (
    input        clk,
    input        rst_n,
    input        newd,
    input [6:0]  addr,
    input        op,          // 0=write, 1=read
    input        mode_sel,    // 0=1-byte, 1=4-byte
    input [7:0]  reg_addr,    // 8-bit register address
    input        sda_in,
    output       sda_out,
    output reg   sda_en,
    output       scl,
    input [31:0] din,
    input [15:0] clk_div,
    output [31:0] dout,
    output reg   busy,
    output reg   ack_err,
    output reg   done,
// Trace Interface
//--------------------------------------------------
output logic [7:0]  trace_event_o,
output logic [31:0] trace_data_o
);

// ------------------------------------------------------------
// Clock divider: 40 MHz / 100 kHz I2C
// 1 I2C bit = 400 sys clocks = 4 x 100-clock quarters
// ------------------------------------------------------------
// Programmable Clock Divider
logic [15:0] clk_div_value;

logic [15:0] CLK1;
logic [15:0] CLK2;
logic [15:0] CLK3;
logic [15:0] CLK4;

assign clk_div_value = (clk_div < 16'd4) ? 16'd100 : clk_div;

assign CLK1 = clk_div_value - 16'd1;
assign CLK2 = (clk_div_value << 1) - 16'd1;
assign CLK3 = (clk_div_value + (clk_div_value << 1)) - 16'd1;
assign CLK4 = (clk_div_value << 2) - 16'd1;


    // ------------------------------------------------------------
    // Quarter-period pulse generator
    //   pulse 0 : SCL low  Q1
    //   pulse 1 : SCL low  Q2  <- drive SDA here
    //   pulse 2 : SCL high Q3  <- sample SDA here
    //   pulse 3 : SCL high Q4
    // ------------------------------------------------------------
    reg [15:0] count1;
    reg [1:0]  pulse;
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
begin
    
pulse  <= 2'd0;
    count1 <= 16'd0;
end
else if(!busy)
begin
    pulse  <= 2'd0;
    count1 <= 16'd0;
end
else
begin
        if      (count1 == CLK1) begin pulse <= 2'd1; count1 <= count1 + 16'd1; end
        else if (count1 == CLK2) begin pulse <= 2'd2; count1 <= count1 + 16'd1; end
        else if (count1 == CLK3) begin pulse <= 2'd3; count1 <= count1 + 16'd1; end
        else if (count1 == CLK4) begin pulse <= 2'd0; count1 <= 16'd0;         end
        else                          count1 <= count1 + 16'd1;
    end
end

// ------------------------------------------------------------
// SDA read-back: when master releases SDA (sda_en=0),
// the bus is pulled high externally. Any 1'bz on SDA
// is treated as logic 1 (pulled high by RPU).
// Using this safe sampled version prevents X propagation
// into rx_data shift register.
// ------------------------------------------------------------

// FSM states
//
// WRITE: START->WR_SLVADDR(W)->ACK_SA->WR_REGADDR->ACK_RA
//        ->WR_DATA->ACK_WR->[repeat for 4-byte]->STOP
//
// READ:  START->WR_SLVADDR(R)->ACK_SA->WR_REGADDR->ACK_RA
//        ->RD_DATA->[MSTR_ACK->RD_DATA repeat]->MSTR_NACK->STOP
// ------------------------------------------------------------
parameter [3:0] IDLE       = 4'd0;
parameter [3:0] START      = 4'd1;
parameter [3:0] WR_SLVADDR = 4'd2;
parameter [3:0] ACK_SA     = 4'd3;
parameter [3:0] WR_REGADDR = 4'd4;
parameter [3:0] ACK_RA     = 4'd5;
parameter [3:0] WR_DATA    = 4'd6;
parameter [3:0] ACK_WR     = 4'd7;
parameter [3:0] RD_DATA    = 4'd8;
parameter [3:0] MSTR_ACK   = 4'd9;
parameter [3:0] MSTR_NACK  = 4'd10;
parameter [3:0] STOP       = 4'd11;

// Trace Event IDs
//--------------------------------------------------
parameter [7:0] TRACE_NONE         = 8'h00;
parameter [7:0] TRACE_START        = 8'h01;
parameter [7:0] TRACE_ADDR_SENT    = 8'h02;
parameter [7:0] TRACE_REGADDR_SENT = 8'h03;
parameter [7:0] TRACE_WRITE_DONE   = 8'h04;
parameter [7:0] TRACE_READ_DONE    = 8'h05;
parameter [7:0] TRACE_ACK_ERROR    = 8'h06;
parameter [7:0] TRACE_STOP         = 8'h07;

    reg [3:0] state;
    reg        scl_t;
    reg        sda_t;
    reg [3:0]  bitcount;
    reg [1:0]  byte_idx;
    reg [7:0]  sh_addr_w;   // {addr, 1'b0}
    reg [7:0]  sh_addr_r;   // {addr, 1'b1}
    reg [7:0]  sh_regaddr;
    reg [31:0] sh_tx;
    reg [31:0] rx_data;
    reg [31:0] dout_r;      // registered output - only updates on read complete
    reg        r_ack;

// Trace Registers
//--------------------------------------------------
logic [7:0]  trace_event_reg;
logic [31:0] trace_data_reg;

    // TX byte mux
// ------------------------------------------------------------
wire [7:0] tx_byte;
assign tx_byte = (mode_sel == 1'b0) ? sh_tx[31:24]   :
                 (byte_idx == 2'd0) ? sh_tx[31:24]   :
                 (byte_idx == 2'd1) ? sh_tx[23:16]   :
                 (byte_idx == 2'd2) ? sh_tx[15:8]    :
                                      sh_tx[7:0];

    // Last byte flag
wire last_byte;
assign last_byte = (mode_sel == 1'b0) ? (byte_idx == 2'd0)
                                      : (byte_idx == 2'd3);
// ------------------------------------------------------------
// Main FSM
// ------------------------------------------------------------
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        state      <= IDLE;
        scl_t      <= 1'b1;
        sda_t      <= 1'b1;
        sda_en     <= 1'b0;
        bitcount   <= 4'd0;
        byte_idx   <= 2'd0;
        sh_addr_w  <= 8'd0;
        sh_addr_r  <= 8'd0;
        sh_regaddr <= 8'd0;
        sh_tx      <= 32'd0;
        rx_data    <= 32'd0;
        dout_r     <= 32'd0;
        r_ack      <= 1'b0;
        busy       <= 1'b0;
        ack_err    <= 1'b0;
        done       <= 1'b0;
        trace_event_reg <= 8'h00;
        trace_data_reg  <= 32'h00000000;
    end else begin
        case (state)

            // ------------------------------------------------------------
            // IDLE: wait for new transaction
            // ------------------------------------------------------------
            IDLE: begin
                ack_err <= 1'b0;
                busy    <= 1'b0;
                if (newd)
    done <= 1'b0;

                scl_t   <= 1'b1;
                sda_t   <= 1'b1;
                sda_en  <= 1'b0;
                    if (newd) begin
                    sh_addr_w  <= {addr, 1'b0};
                    sh_addr_r  <= {addr, 1'b1};
                    sh_regaddr <= reg_addr;
                    sh_tx      <= din;
                    rx_data    <= 32'd0;
                    byte_idx   <= 2'd0;
                    bitcount   <= 4'd0;
                    busy       <= 1'b1;
                    //--------------------------------------------------
         // Trace : Transaction Start
            trace_event_reg <= TRACE_START;
            trace_data_reg  <= {29'd0, mode_sel, op, newd};
                    sda_en     <= 1'b1;   // FIX: pre-set here so sda_en rises on the same edge state->START (was lagging 1 clk)
                    state      <= START;
                end else begin
                    busy <= 1'b0;
                                end
                end
       
            // ------------------------------------------------------------
            // START: SDA falls while SCL is high
            // ------------------------------------------------------------
            START: begin
                sda_en <= 1'b1;
                case (pulse)
                    2'd0: begin scl_t <= 1'b1; sda_t <= 1'b1; end
                    2'd1: begin scl_t <= 1'b1; sda_t <= 1'b1; end
                    2'd2: begin scl_t <= 1'b1; sda_t <= 1'b0; end // START
                    2'd3: begin scl_t <= 1'b1; sda_t <= 1'b0; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t    <= 1'b0;
                    bitcount <= 4'd0;
                    state    <= WR_SLVADDR;
                end
            end
            // ------------------------------------------------------------
            // WR_SLVADDR: 8-bit slave address + R/W bit
            //   op=0  ->  addr|W  (bit0 = 0)
            //   op=1  ->  addr|R  (bit0 = 1)
            // ------------------------------------------------------------
            WR_SLVADDR: begin
                sda_en <= 1'b1;
                if (bitcount <= 4'd7) begin
                    case (pulse)
                        2'd0: begin scl_t <= 1'b0; end
                        2'd1: begin
                            scl_t <= 1'b0;
                            sda_t <= (op == 1'b0)
                ? sh_addr_w[3'd7 - bitcount[2:0]]
                : sh_addr_r[3'd7 - bitcount[2:0]];
                        end
                        2'd2: begin scl_t <= 1'b1; end
                        2'd3: begin scl_t <= 1'b1; end
                        default: ;
                    endcase
                    if (count1 == CLK4) begin
                        scl_t <= 1'b0;
                        // FIX: fold the byte-complete transition into this same edge
                        // (previously it waited one extra cycle for bitcount to roll
                        // over to 8, so sda_en/state trailed scl_t by 1 clock here).
                        if (bitcount == 4'd7) begin
                            bitcount <= 4'd0;
                            sda_en   <= 1'b0;   // release for slave ACK
                            state    <= ACK_SA;
                        end else begin
                            bitcount <= bitcount + 4'd1;
                        end
                    end
                end
            end

            // ------------------------------------------------------------
            // ACK_SA: receive slave ACK after address byte
            // ------------------------------------------------------------
            ACK_SA: begin
                sda_en <= 1'b0;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; end
                    2'd1: begin scl_t <= 1'b0; end
                    2'd2: begin scl_t <= 1'b1; r_ack <= sda_in; end  // sample
                    2'd3: begin scl_t <= 1'b1; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t <= 1'b0;
                    if (r_ack == 1'b0) begin         // ACK = SDA low
                     // Trace : Slave Address Sent (ADDR Register)
                   //--------------------------------------------------
                    trace_event_reg <= TRACE_ADDR_SENT;
                    trace_data_reg  <= {25'd0, addr};
                        bitcount <= 4'd0;
                        sda_en   <= 1'b1;
                        state    <= WR_REGADDR;
                    end else begin                   // NACK -> abort
                        ack_err <= 1'b1;
                        //--------------------------------------------------
                       // Trace : STATUS Register (ACK Error)
                       //--------------------------------------------------
                       trace_event_reg <= TRACE_ACK_ERROR;
                       trace_data_reg  <= {29'd0, 1'b1, done, busy};
                        sda_en  <= 1'b1;
                        sda_t   <= 1'b0;   // FIX: pre-set to match STOP's pulse-0 entry value
                        state   <= STOP;
                    end
                end
            end

            // ------------------------------------------------------------
            // WR_REGADDR: 8-bit register address
            // Applies to both read and write transactions
            // Slave latches this as its internal pointer
            // ------------------------------------------------------------
            WR_REGADDR: begin
                sda_en <= 1'b1;
                if (bitcount <= 4'd7) begin
                    case (pulse)
                        2'd0: begin scl_t <= 1'b0; end
                        2'd1: begin
                            scl_t <= 1'b0;
                            sda_t <= sh_regaddr[3'd7 - bitcount[2:0]];
                        end
                        2'd2: begin scl_t <= 1'b1; end
                        2'd3: begin scl_t <= 1'b1; end
                        default: ;
                    endcase
                    if (count1 == CLK4) begin
                        scl_t <= 1'b0;
                        // FIX: fold the byte-complete transition into this same edge
                        // (previously it waited one extra cycle for bitcount to roll
                        // over to 8, so sda_en/state trailed scl_t by 1 clock here).
                        if (bitcount == 4'd7) begin
                            bitcount <= 4'd0;
                            sda_en   <= 1'b0;   // release for slave ACK
                            state    <= ACK_RA;
                        end else begin
                            bitcount <= bitcount + 4'd1;
                        end
                    end
                end
            end

            // ------------------------------------------------------------
            // ACK_RA: receive slave ACK after reg address
            //   op=0  ->  WR_DATA  (write path)
            //   op=1  ->  RD_DATA  (read path)
            // ------------------------------------------------------------
            ACK_RA: begin
                sda_en <= 1'b0;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; end
                    2'd1: begin scl_t <= 1'b0; end
                    2'd2: begin scl_t <= 1'b1; r_ack <= sda_in; end  // sample
                    2'd3: begin scl_t <= 1'b1; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t    <= 1'b0;
                    bitcount <= 4'd0;
                    if (r_ack == 1'b0) begin
                     // Trace : REG_ADDR Register
                     //--------------------------------------------------
                     trace_event_reg <= TRACE_REGADDR_SENT;
                     trace_data_reg  <= {24'd0, reg_addr};
                        if (op == 1'b0) begin
                            sda_en <= 1'b1;
                            state  <= WR_DATA;   // WRITE path
                        end else begin
                            sda_en <= 1'b0;      // release SDA for slave
                            state  <= RD_DATA;   // READ path
                        end
                    end else begin
                        ack_err <= 1'b1;
                        //--------------------------------------------------
                        // Trace : STATUS Register (ACK Error)
                        //--------------------------------------------------
                        trace_event_reg <= TRACE_ACK_ERROR;
                        trace_data_reg  <= {29'd0, 1'b1, done, busy};
                        sda_en  <= 1'b1;
                        sda_t   <= 1'b0;   // FIX: pre-set to match STOP's pulse-0 entry value
                        state   <= STOP;
                    end
                end
            end

            // ------------------------------------------------------------
            // WR_DATA: send one data byte, MSB first
            // ------------------------------------------------------------
            WR_DATA: begin
                sda_en <= 1'b1;
                if (bitcount <= 4'd7) begin
                    case (pulse)
                        2'd0: begin scl_t <= 1'b0; end
                        2'd1: begin
                            scl_t <= 1'b0;
                            sda_t <= tx_byte[3'd7 - bitcount[2:0]];
                        end
                        2'd2: begin scl_t <= 1'b1; end
                        2'd3: begin scl_t <= 1'b1; end
                        default: ;
                    endcase
                    if (count1 == CLK4) begin
                        scl_t <= 1'b0;
                        // FIX: fold the byte-complete transition into this same edge
                        // (previously it waited one extra cycle for bitcount to roll
                        // over to 8, so sda_en/state trailed scl_t by 1 clock here).
                        if (bitcount == 4'd7) begin
                            bitcount <= 4'd0;
                            sda_en   <= 1'b0;   // release for slave ACK
                            state    <= ACK_WR;
                        end else begin
                            bitcount <= bitcount + 4'd1;
                        end
                    end
                end
            end

            // ------------------------------------------------------------
            // ACK_WR: receive slave ACK after write byte
            //   last byte  ->  STOP
            //   more bytes ->  byte_idx++ -> WR_DATA
            // ------------------------------------------------------------
            ACK_WR: begin
                sda_en <= 1'b0;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; end
                    2'd1: begin scl_t <= 1'b0; end
                    2'd2: begin scl_t <= 1'b1; r_ack <= sda_in; end  // sample
                    2'd3: begin scl_t <= 1'b1; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t <= 1'b0;
                    if (r_ack == 1'b0) begin
                        if (last_byte) begin
                            sda_en <= 1'b1;
                            sda_t  <= 1'b0;   // FIX: pre-set to match STOP's pulse-0 entry value
                         
                          // Trace : DATA_IN Register
                         //--------------------------------------------------
                         trace_event_reg <= TRACE_WRITE_DONE;
                         trace_data_reg  <= din;  
                          state  <= STOP;           // all bytes sent
                        end else begin
                            byte_idx <= byte_idx + 2'd1;
                            bitcount <= 4'd0;
                            sda_en   <= 1'b1;
                            state    <= WR_DATA;      // next byte
                        end
                    end else begin
                        ack_err <= 1'b1;
                        //--------------------------------------------------
                       // Trace : STATUS Register (ACK Error)
                       //--------------------------------------------------
                        trace_event_reg <= TRACE_ACK_ERROR;
                        trace_data_reg  <= {29'd0, 1'b1, done, busy};
                        sda_en  <= 1'b1;
                        sda_t   <= 1'b0;   // FIX: pre-set to match STOP's pulse-0 entry value
                        state   <= STOP;
                    end
                end
            end

            // ------------------------------------------------------------
            // RD_DATA: clock in one byte from slave, MSB first
            //   master holds SCL low between bits
            //   slave drives SDA; master samples at pulse 2
            // ------------------------------------------------------------
            RD_DATA: begin
                sda_en <= 1'b0;        // master releases SDA; slave drives
                if (bitcount <= 4'd7) begin
                    case (pulse)
                        2'd0: begin scl_t <= 1'b0; end
                        2'd1: begin scl_t <= 1'b0; end
                        2'd2: begin
                            scl_t <= 1'b1;

                            if (count1 == CLK3) begin
/*
                                $display("[%0t] SAMPLE bit=%0d byte=%0d sda=%0b",
                                         $time,
                                         bitcount,
                                         byte_idx,
                                         sda_in);
*/
                                if(mode_sel == 1'b0)
                                    rx_data[7:0] <= {rx_data[6:0], sda_in};
                                else begin
                                    case(byte_idx)
                                        2'd0: rx_data[31:24] <= {rx_data[30:24], sda_in};
                                        2'd1: rx_data[23:16] <= {rx_data[22:16], sda_in};
                                        2'd2: rx_data[15:8]  <= {rx_data[14:8],  sda_in};
                                        2'd3: rx_data[7:0]   <= {rx_data[6:0],   sda_in};
                                        default: ;
                                    endcase
                                end
                            end
                        end
                        2'd3: begin scl_t <= 1'b1; end

                        default: ;
                    endcase
                    if (count1 == CLK4) begin
                        scl_t <= 1'b0;
                        // FIX: fold the byte-complete transition into this same edge
                        // (previously it waited one extra cycle for bitcount to roll
                        // over to 8, so sda_en/state trailed scl_t by 1 clock here).
                        if (bitcount == 4'd7) begin
                            bitcount <= 4'd0;
                            sda_en   <= 1'b1;
                            sda_t    <= last_byte ? 1'b1 : 1'b0;  // FIX: pre-set to match MSTR_NACK(1)/MSTR_ACK(0) entry value
                            state    <= last_byte ? MSTR_NACK : MSTR_ACK;
                        end else begin
                            bitcount <= bitcount + 4'd1;
                        end
                    end
                end
            end

            // ------------------------------------------------------------
            // MSTR_ACK: master sends ACK (SDA=0) between
            //           multi-byte reads; then next byte
            // ------------------------------------------------------------
            MSTR_ACK: begin
                sda_en <= 1'b1;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; sda_t <= 1'b0; end
                    2'd1: begin scl_t <= 1'b0; sda_t <= 1'b0; end
                    2'd2: begin scl_t <= 1'b1; sda_t <= 1'b0; end
                    2'd3: begin scl_t <= 1'b1; sda_t <= 1'b0; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t    <= 1'b0;
                    byte_idx <= byte_idx + 2'd1;
                    bitcount <= 4'd0;
                    sda_en   <= 1'b0;
                    state    <= RD_DATA;
                end
            end

            // ------------------------------------------------------------
            // MSTR_NACK: master sends NACK (SDA=1) after
            //            last read byte -> then STOP
            // ------------------------------------------------------------
            MSTR_NACK: begin
                sda_en <= 1'b1;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; sda_t <= 1'b1; end
                    2'd1: begin scl_t <= 1'b0; sda_t <= 1'b1; end
                    2'd2: begin scl_t <= 1'b1; sda_t <= 1'b1; end
                    2'd3: begin scl_t <= 1'b1; sda_t <= 1'b1; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    // Latch final rx_data into dout register
                    // before transitioning to STOP
                    /*
                    $display("[%0t] FINAL rx_data=%h dout_r=%h",
                             $time,
                             rx_data, dout_r);
                    $display("[%0t] LATCHING rx_data=%h into dout_r",
                             $time,
                             rx_data);
                    */
                    dout_r <= rx_data;
                     // Trace : DATA_OUT Register
                    //--------------------------------------------------
                    trace_event_reg <= TRACE_READ_DONE;
                    trace_data_reg  <= rx_data;
                    scl_t  <= 1'b0;
                    sda_t  <= 1'b0;   // pre-drive low for clean STOP transition
                    sda_en <= 1'b1;
                    state  <= STOP;
                end
            end

            // ------------------------------------------------------------
            // STOP: SDA rises while SCL is high
            // ------------------------------------------------------------
            STOP: begin
                sda_en <= 1'b1;
                case (pulse)
                    2'd0: begin scl_t <= 1'b0; sda_t <= 1'b0; end
                    2'd1: begin scl_t <= 1'b1; sda_t <= 1'b0; end  // SCL rises first
                    2'd2: begin scl_t <= 1'b1; sda_t <= 1'b1; end  // SDA rises -> STOP
                    2'd3: begin scl_t <= 1'b1; sda_t <= 1'b1; end
                    default: ;
                endcase
                if (count1 == CLK4) begin
                    scl_t  <= 1'b1;
                    sda_t  <= 1'b1;
                    sda_en <= 1'b0;
                    busy   <= 1'b0;
                    done   <= 1'b1;
// Trace : STATUS Register (Transaction Complete)
        //--------------------------------------------------
        trace_event_reg <= TRACE_STOP;
        trace_data_reg <= {29'd0, ack_err, done, busy};
                    
                    state  <= IDLE;
                end
            end

            default: state <= IDLE;
        endcase
    end
end

// ------------------------------------------------------------
// Tri-state SDA driver
//   sda_en=1 & sda_t=0  ->  pull SDA low
//   sda_en=1 & sda_t=1  ->  high-Z (external RPU pulls high)
//   sda_en=0            ->  high-Z (slave may drive freely)
// ------------------------------------------------------------
assign sda_out = (sda_en && (sda_t == 1'b0)) ? 1'b0 : 1'b1;
assign scl     = scl_t;

// dout is registered and only updates when a read completes
// (latched in MSTR_NACK). This prevents X/glitch on the
// APB read bus between transactions.
assign dout = dout_r;
// Trace Output Connections
//--------------------------------------------------
assign trace_event_o = trace_event_reg;
assign trace_data_o  = trace_data_reg;

endmodule

