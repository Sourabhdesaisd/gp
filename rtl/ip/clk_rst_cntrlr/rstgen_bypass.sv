...
//    logic [NumRegs-1:0] synch_regs_q;
    ...
    tc_clk_mux2 i_tc_clk_mux2_rst_no (
        .clk0_i     ( rst_sync_q3), //synch_regs_q[NumRegs-1] ),
        .clk1_i     ( rst_test_mode_ni ),
        .clk_sel_i  ( test_mode_i ),
        .clk_o      ( rst_nof )
    );
...
*/
    always_ff@(posedge clk_i or negedge por_ni) begin
        if(!por_ni) begin
            ...
            rst_sync_q2 <= 1'b0;
            rst_sync_q3 <= 1'b0;
//            $display($time,"============rst change 0======== rst_n=%d", rst_n);
        end else begin
            rst_sync_q1 <= rst_n;
            rst_sync_q2 <= rst_sync_q1;
            rst_sync_q3 <= rst_sync_q2;
//            $display($time,"============rst change 1=========================================== rst_n=%d", rst_n);
...
//    assign rst_no = rst_nof; //(rst_nof && rst_sync_q2);
