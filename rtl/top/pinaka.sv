...
/*
//          POST BOOT CONFIG REGISTER OUT TO THE ANALOG TOP FROM THE ANALOG REGISTER

output      [2:0]       pll_config_reg_out              ,                          
output      [31:0]      ds_adc_config0_reg_out          ,                     
output      [15:0]      ds_adc_config1_reg_out          ,

//          POST BOOT TRIM REGISTER OUT TO THE ANALOG TOP FROM THE ANALOG REGISTER

output      [17:0]      ds_adc_trim_reg_out             ,                         
output      [2:0]       sar_adc_trim_reg_out            ,                               
output      [11:0]      vtc_tdc_trim_reg_out            ,                        
output      [2:0]       cs_dac_trim_reg_out             ,                         
output      [2:0]       c_dac_trim_reg_out              ,                         
output      [2:0]       opamp3v3_trim_reg_out           ,                        
output      [2:0]       opampint_trim_reg_out           ,                          
output      [2:0]       opamp2v5_trim_reg_out           ,

//              OTHER REGISTERS TO ANALOG TOP 

output                  bg_rstb_mxx_reg_out             ,
output                  cdac_rstb_mxx_reg_out           ,
output      [11:0]      cdac_vin_mxx_reg_out            ,
output                  cifb_rstb_mxx_reg_out           ,
output                  ciff_rstb_mxx_reg_out           ,
output                  csdac_rstb_mxx_reg_out          ,
output      [11:0]      csdac_vin_mxx_reg_out           ,
output                  ldo_12a_rstb_mxx_reg_out        ,
output                  ldo_12b_rstb_mxx_reg_out        ,
output                  ldo_18a_rstb_mxx_reg_out        ,
output                  ldo_18b_rstb_mxx_reg_out        ,
output                  ldo_25a_rstb_mxx_reg_out        ,
output                  ldo_25b_rstb_mxx_reg_out        ,
output                  osc_rstb_mxx_reg_out            ,
output                  pll_rstb_mxx_reg_out            ,
output                  por_rstb_mxx_reg_out            ,
output                  sar_rstb_mxx_reg_out            ,
output                  tdc_rstb_mxx_reg_out            ,
output                  vmon_rst_mxx_reg_out            ,

//          PRE BOOT ANALOG REGISTER TO ANALO TOP

//output      [4:0]       block_sel_reg_out               ,
//output      [1:0]       mode_sel_reg_out                ,
...
output      [47:0]      analog_postboot_reg1            ,
...
//                  TO THE IO PAD BLOCKS

output      [31:0]      gpio_pullup_out                 ,                   
output      [31:0]      gpio_pulldown_out               ,                        
output      [31:0]      gpio_opendrain_out              ,                          
output      [31:0]      gpio_scmitt_out                 ,                          
output      [31:0]      gpio_drv0_out                   ,                            
output      [31:0]      gpio_drv1_out                   ,                              
output      [31:0]      gpio_int_en_out                 ,                             
output      [31:0]      gpio_int_status_out             ,                         
output      [31:0]      gpio_rise_en_out                ,                         
output      [31:0]      gpio_fall_en_out                ,                          
output      [31:0]      gpio_high_en_out                ,                           
output      [31:0]      gpio_low_en_out                 ,                            
output      [31:0]      gpio_lock_out                   ,                             
output      [31:0]      gpio_sel_out             
...
wire                                    core_int_ctrl_ack_read_valid        ;
wire                                    core_int_ctrl_eoi_write_valid       ;
wire    [7:0]                           core_int_ctrl_eoi_id                ;

//----------------------------------CSR INTERRUPT CONTROLLER SIGNALS-------------------------

wire    [7:0]                           int_ctrl_csr_highest_pending_lvl_pr ;
wire    [7:0]                           int_ctrl_csr_ack_id                 ;
...
wire    [19:0]                          core_debug_trace_iaddr              ;
wire    [7:0]                           core_debug_trace_cause              ;
wire    [2:0]                           core_debug_trace_priv               ;
wire    [2:0]                           core_debug_trace_context            ;
...
wire           [19:0]                   csr_debug_trace_tval                ;
...
assign  core_mmu_inst_req                       =   ~boot_load_enable & ~mmu_busy                          ;
...
//                          FROM THE CLK RST CONTROLLER MODULE
            ...
            .debug_clk_in                       (ctrl_bypass_debug_clk)                         ,
            .mem_clk_in                         (core_clk)                                      ,
...
//                              TO THE DEBUG MODULE
            ...
            .mux_jtag_debug_pulse_out           (debug_rstn)                                    ,
...
//                      FROM THE AXI-APB BRIDGE MODULE
            ...
            .pwdata_i                           ({bridge_peripheral_pwdata[31],bridge_peripheral_pwdata[5:0]})                      ,
...
//                          TO THE CLK RST BYPASS MODULE
            ...
            .clk_for_debug_o                    (ctrl_bypass_debug_clk)                         ,
...
//                                  TO THE INTERRUPT CONTROLLER MODULE
            ...
            .ack_read_valid_o                   (core_int_ctrl_ack_read_valid)                  ,       
...
//                              FROM THE CORE
            ...
            .soc_ack_read_valid_en	            (core_int_ctrl_ack_read_valid)                  ,
...
//
//                          THIS IS NOT FIXED, NEED TO CONFORM
...
//                                      TO THE CSR MODULE
            ...
            .highest_pending_lvl_pr_o	        (int_ctrl_csr_highest_pending_lvl_pr)           ,
            .soc_ack_int_id_o		            (int_ctrl_csr_ack_id)                           ,
...
//                              FROM THE INTERRUPT CONTROLLER MODULE
           ...
           .interrupt_code_in			        ({{24{1'b0}},int_ctrl_csr_ack_id})	            ,
           .interrupt_lvl_pr_i			        (int_ctrl_csr_highest_pending_lvl_pr)		    ,
...
//                      FROM THE CLK RST CONTROLLER MODULE
            ...
            .pclk                               (pclk)                                          ,
            .clk_out_w                          (ctrl_bypass_debug_clk)                         ,
...
//                          TO THE TOP OUTPUT PINS
            ...
            .gpio_pullup_out_w                  (gpio_pullup_out)                               , 
            .gpio_pulldown_out_w                (gpio_pulldown_out)                             ,
            .gpio_opendrain_out_w               (gpio_opendrain_out)                            ,
                  
            .gpio_schmitt_out_w                 (gpio_scmitt_out)                               ,
                   
            .gpio_drv0_out_w                    (gpio_drv0_out)                                 ,
            .gpio_drv1_out_w                    (gpio_drv1_out)                                 ,
                   
            .gpio_int_en_out_w                  (gpio_int_en_out)                               ,
            .gpio_int_status_out_w              (gpio_int_status_out)                           ,
                   
            .gpio_rise_en_out_w                 (gpio_rise_en_out)                              ,
            .gpio_fall_en_out_w                 (gpio_fall_en_out)                              ,
                   
            .gpio_high_en_out_w                 (gpio_high_en_out)                              ,
            .gpio_low_en_out_w                  (gpio_low_en_out)                               ,
                   
            .gpio_lock_out_w                    (gpio_lock_out)                                 ,
            .debug_sel_out_w                    (gpio_sel_out)                                  ,
...
//                                  CONFIG REGISTERS
            ...
            .pll_config_reg                     (ppl_config_reg_out)                            ,
            .ds_adc_config0_reg                 (ds_adc_config0_reg_out)                        ,
            .ds_adc_config1_reg                 (ds_adc_config1_reg_out)                        ,

//                                  TRIM REGISTERS

            .ds_adc_trim_reg                    (ds_adc_trim_reg_out)                           ,
            .sar_adc_trim_reg                   (sar_adc_trim_reg_out)                          ,
            .vtc_tdc_trim_reg                   (vtc_tdc_trim_reg_out)                          ,
            .cs_dac_trim_reg                    (cs_dac_trim_reg_out)                           ,
            .c_dac_trim_reg                     (c_dac_trim_reg_out)                            ,
            .opamp3v3_trim_reg                  (opamp3v3_trim_reg_out)                         ,
            .opampint_trim_reg                  (opampint_trim_reg_out)                         ,
            .opamp2v5_trim_reg                  (opamp2v5_trim_reg_out)                         ,

//                          OTHER REGISTER TO THE ANALOG TOP

            .bg_rstb_mxx                        (bg_rstb_mxx_reg_out)                           ,
            .cdac_rstb_mxx                      (cdac_rstb_mxx_reg_out)                         ,
            .cifb_rstb_mxx                      (cifb_rstb_mxx_reg_out)                         ,
            .ciff_rstb_mxx                      (ciff_rstb_mxx_reg_out)                         ,
            .csdac_rstb_mxx                     (csdac_rstb_mxx_reg_out)                        ,
            .ldo12a_rstb_mxx                    (ldo_12a_rstb_mxx_reg_out)                      ,
            .ldo12b_rstb_mxx                    (ldo_12b_rstb_mxx_reg_out)                      ,
            .ldo18a_rstb_mxx                    (ldo_18a_rstb_mxx_reg_out)                      ,
            .ldo18b_rstb_mxx                    (ldo_18b_rstb_mxx_reg_out)                      ,
            .ldo25a_rstb_mxx                    (ldo_25a_rstb_mxx_reg_out)                      ,
            .ldo25b_rstb_mxx                    (ldo_25b_rstb_mxx_reg_out)                      ,
            .osc_rstb_mxx                       (osc_rstb_mxx_reg_out)                          ,
            .pll_rstb_mxx                       (pll_rstb_mxx_reg_out)                          ,
            .por_rstb_mxx                       (por_rstb_mxx_reg_out)                          ,
            .sar_rstb_mxx                       (sar_rstb_mxx_reg_out)                          ,
            .tdc_rstb_mxx                       (tdc_rstb_mxx_reg_out)                          ,
            .vmon_rstb_mxx                      (vmon_rst_mxx_reg_out)                          ,
            .cdac_vin_mxx                       (cdac_vin_mxx_reg_out)                          ,           
            .csdac_vin_mxx                      (csdac_vin_mxx_reg_out)                         ,
*/
...
//                          FROM THE CLK RST CONTROLLER MODULE
            ...
            .debug_rst                          (ctrl_bypass_debug_rstn)                        ,
...
//                          FROM THE CLK RST CONTROLLER MODULE
            ...
            .it_clk                             (ctrl_bypass_debug_clk)                         ,
            .it_rst_n                           (ctrl_bypass_debug_rstn)                        ,

            .core_clk_i                         (core_clk)                                      ,                     
            .core_rst_ni                        (core_rstn)                                     ,
...
//                          FROM THE AXI-APB BRIDGE MODULE
            ...
            .penable_i                          (bridge_peripheral_penable)                     ,
            .pwrite_i                           (bridge_peripheral_pwrite)                      ,
            .pwdata_i                           (bridge_peripheral_pwdata[0])                   ,
