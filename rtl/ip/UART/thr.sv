...
*/
    ...
    always_ff @(posedge clk_r or negedge rst_n) begin
      ...
      //  else if (tx_clr_rsync) tgl_r <= 1'b0;
        else if (tsr_load && rd_count==mdr_1-5'd2)     tgl_r <= ~tgl_r;
    end
    ...
    assign internal_read_enable = tsr_load & (rd_count == mdr_1-5'd2);

    always_ff @(posedge clk_r or negedge rst_n) begin
        if (!rst_n) begin
            rd_count <= '0;
        end 
         else if (tsr_load) begin
            if (rd_count == mdr_1-5'd1)
                rd_count <= '0;
            else
...
logic do_read;
   ...
   assign thr_empty = ~thr_full2;
    
 assign do_read  = internal_read_enable & ~thr_empty;
